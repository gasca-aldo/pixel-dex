...
export function listSnapshot(list: GameList) {
  ...
  return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(p.title)} · Pixel Tracker</title><style>body{max-width:780px;margin:40px auto;padding:24px;font:16px/1.7 system-ui;background:#111115;color:#eeedf5}article{padding:24px 0;border-bottom:1px solid #34323e}p{white-space:pre-wrap}small{color:#aaa}h1{font-size:36px}</style><small>Pixel Tracker · List snapshot</small><h1>${escapeHtml(p.title)}</h1><p>${escapeHtml(p.description)}</p>${p.entries.map((e, i) => `<article><h2>${p.ranked ? `${i + 1}. ` : ''}${escapeHtml(e.game.title)}</h2><p>${escapeHtml(e.note)}</p></article>`).join('')}</html>`;
}
